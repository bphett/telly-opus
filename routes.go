...
func stream(lineup *lineup) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if channel, ok := lineup.channels[channelID]; ok {
			...
			run := exec.Command("ffmpeg", "-fflags", "+genpts", "-i", "pipe:0", "-async", "1", "-fps_mode", "cfr", "-c:v", "libx264", "-preset", "veryfast", "-tune", "zerolatency", "-c:a", "libopus", "-bsf:v", "filter_units=remove_types=6", "-f", "mpegts", "pipe:1")
